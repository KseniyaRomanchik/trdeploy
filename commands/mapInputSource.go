...
func prepareNestedInputSource(mic altsrc.InputSourceContext, path string, flags []cli.Flag) altsrc.InputSourceContext {
	var newValueMap = make(map[interface{}]interface{}, len(flags))

	for _, f := range flags {
		flagname := f.Names()[0]
		value, err := mic.String(path + "." + flagname)
		if err != nil || value == "" {
			continue
		}
